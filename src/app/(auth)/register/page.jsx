...
export default function RegisterPage() {
  ...
  return (
    <div className="max-w-md mx-auto py-40 text-black p-6 bg-pink-200 rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold text-green-700 mb-6 text-center">Register</h2>
      {error && <p className="text-red-600 mb-4">{error}</p>}
      <form onSubmit={handleSubmit} className="space-y-4">
        <input name="name" placeholder="Name" value={form.name} onChange={handleChange} required className="w-full p-2 border-2 border-green-300 rounded-md focus:ring-2 focus:ring-green-500" />
        ...
        <button type="submit" disabled={loading} className="w-full py-2 bg-green-500 hover:bg-green-600 text-white font-bold rounded-md">{loading ? "Registering..." : "Register"}</button>
      </form>
    </div>
  );
}
